...
\title{Multi-spectral Image Representation for Image Segmentation Tasks} 
%\title{Multi-spectral Image Representation for Segmentation Tasks and High-level Texture Features} 
%\title{Perceptual Image Analsys Based on Multi-spectral Image Decomposition} 
...
\end{equation}
represents the filter response at different central frequencies $f$ (scales) and orientations $\theta$. Given the complex form of Gabor filters, the filter response $r_{f,\theta}(x, y)$ has a real and an imaginary part, here denoted as $\RE{(\cdot)}$ and $\IM{(\cdot)}$, respectively.
...
%\textit{Idea to develop:} Illustrate the effect of computing uni-chrome features in the gray-scale and the RGB space for a color image.
...
Notwithstanding, we find a large number of methods that propose the characterization of textures in color images. Such methods generally use two strategies for the analysis of color textures \cite{Maenpaa.Pietikainen:PR:2004}, \cite{Qazi.Alata.ea:PR:2011}:
...
The choice of texture-forming colors comes from the interest in visualizing the color spectrum of the image more graphically. The two-channel color spaces encode color information in a complex chrominance channel. Therefore, we can interpret the chrominance values as points within a complex plane (see Fig. \ref{fig:color_complex_plane}). In the case of LAB/LUV spaces, the values of chrominance are defined in Cartesian coordinates as Eq. \eqref{eq:chrominance_lab} where channel $A$ values represent the real axis coordinates, and channel $B$ values represent the imaginary axis coordinates. HSV/HSL color spaces encode chrominance values as points in polar coordinates as Eq. \eqref{eq:chrominance_hsv} where channel $H$ values are the angle expressed radians and channel $S$ values are the distance from the origin to the color points.
...
%Gabor responses after morphological opening appear in the figure \ref{fig:synthetic_img_gresponses_opn}.
...
%    \end{subfigure} 
%    	    
%    \caption{Gabor responses after the morphological openning of the synthetic image. (a) Luminance channel responses, (b) Real part of chrominance channel responses, (c) Imaginary part of chrominance channel responses.}\label{fig:synthetic_img_gresponses_opn}    
%\end{figure}
...
%Figure \ref{fig:synthetic_img_gresponses_opn_smth} shows the Gabor responses after opening and adaptive Gaussian smoothing. 
...
%    \end{subfigure} \\ [2ex]    	
%    \begin{subfigure}[b]{0.45\textwidth}  
%        \includegraphics[width=\textwidth]{gabor_resp_ci_synthetic_opn_smth}
%        \caption{$\widetilde{e}_{\IM(C), f, \theta}(x,y)$}
%        \label{fig:ci_gabor_energies_opn_smth} 
%    \end{subfigure} 
%    	    
%    \caption{Gabor responses after morphological openning and Gaussian smoothing of the synthetic image. (a) luminance channel responses, (b) real part of chrominance channel responses, (c) imaginary part of chrominance channel responses.}\label{fig:synthetic_img_gresponses_opn_smth}    
%\end{figure}
...
\section{State of the Art and Evaluation of Contour Detectors and Segmentation of Natural Images}\label{sec:soa_boundaries_segmentation}
...
\subsection{Bechmark}\label{subsec:benchmark}
...
Finally, both databases (BSDS300 and BSDS500) contain segmentations of gray level and color images. Since in this paper we analyze the color textures, we mainly use the BSDS500 color images with their respective segmentations to evaluate our segmentation results. 
...
In addition to these scores, the BSDS benchmark uses the following measures and tools to compare results in contour detection and image segmentation.
...
We also quantitatively evaluate the quality of the feature space developed in this paper. For this, we use a database of natural images that have a ground truth generated by humans. The database and its characteristics are described below.
...
We use the graph theory to represent images as connected structures for modeling pairwise relations between objects. We explore mainly two types of structures regarding the support: Pixel- and Region-based graph

\subsubsection{Pixel-based graph representation}

Considering a digital image as a 2-d grid of pixels, where the intensity (or color) values are mapped to the spatial coordinates $(x, y)$, we can represent all the pixels as a dense graph. In the graph theory, $\mathcal{G}=(\mathsf{V}, \mathsf{E})$ is a graph where each node $\mathsf{v}_i \in \mathsf{V}$ corresponds to a pixel in the image, and the edges $\mathsf{e} \in \mathsf{E}$ the link between the image's pixels.
...
We can define the adjacency level of the pixels to generate a specific pixel-based graph. In our applications, we mainly use the 4-neighbor adjacency system (4nn). This configuration is illustrated in figure \ref{fig:pixel_adjacency_graph}.
...
To build this type of graph, we must first separate the image into regions, preferably into regions that are coherent with the image's perceptual information. Subsequently, the graph nodes represent the image regions while the graph edges follow the same strategies described above to connect the regions. The primary type of graph that we consider in this work is the \textit{Region Adjacency Graph }(RAG). This configuration is illustrated in figure \ref{fig:region_adjacency_graph}.

Pixels are the smallest elements in the image. The grouping of these elements into coherent regions generates the so-called superpixels. In the literature, can find different algorithms for the generation of image superpixel that varies on the implementation and performance \cite{Wang.Liu.ea:SP:2017}. In this work, we choose the SLIC algorithm \cite{Stutz.Hermans.ea:CVIU:2018} to generate the image superpixels. This method is one of the most competent in terms of customization and calculation time. 
...
\begin{figure}[!ht]
    ...
    \begin{subfigure}[b]{0.24\textwidth}
        ...
        \caption{}
        \label{fig:pixel_adjacency_graph} 
    \end{subfigure}         
    \begin{subfigure}[b]{0.24\textwidth}
    ...
    \caption{Representation of an image as a graph: (a) Pixel Adjacency Graph (4-neighbor adjacency), (b) Region Adjacency Graph.}\label{fig:image_a_graph} 
\end{figure}

\subsection{Earth Mover's Distance for Non-normalized Distributions}
The EMD has shown its usefulness as a true metric for measuring the similarity between distributions \cite{Bazan.Dokladal.ea:BMVC:2019}. In image retrieval systems based on color information, the EMD can measure tiny changes in the normalized color distributions of images. On the other hand, with the texture patches, the EMD can capture, reflect and measure the importance of the logarithmic (frequency) and polar (orientation) axis of the texture signature. In this paper we use the EMD to measure the similarity between two elements of an image: pixels or superpixels. By measuring the similarity of elements in the Gabor-filter-based feature space, we locally measure changes in color and texture in the image. 

The EMD \cite{Rubner.Tomasi.ea:IJCV:2000}, as used to measure color and texture distributions in \cite{Bazan.Dokladal.ea:BMVC:2019}, is a true metric only when used with normalized distributions (histograms or signatures), i.e., distributions with total mass equal to one. This fact has no impact when the distributions represent global information of an image (color or texture). However, the distributions of the image elements (pixels or superpixels) contain only a portion of the information; for example, in the case of our feature space, Gabor's energy. 
...
As we described earlier, we can use pixels or superpixels as support to represent an image as a graph. In this sense, each node of the graph corresponds to a pixel of the image or, to the centroid of each image superpixel. Since the number of edges increases with the number of nodes, the strategy of calculating the gradient on a region-based graph allows reducing the EMD computation time for the set of edges of an image. This action generates a tradeoff between the final gradient and the regions generated by the SLIC algorithm. We compute the gradients on both supports; for the PAG, we use a four neighboring node adjacency system (4nn), whereas we set the SLIC algorithm to generate 2500 superpixels for the RAG. 

The weight of the edges between nodes of both computational supports (pixels and superpixels)is given by the EMD for non-normalized distributions. 
\begin{equation}
